...
import { connectRows } from "../helpers/commonFunctions/connectRows";
...
export const identifyController = async (req: Request, res: Response) => {
    ...
    let [connectRes, id] = await connectRows(databaseResults.data)
    if (!connectRes) {
        return res.status(500).json({
            errors: ["Issue talking to the database"]
        })
    ...
    if (newInformationGiven) {
        await tryCatch(prisma.contact.create({
            data: {
    ...
    const allRows = await getAllRowsForPrimaryId(id)
    if (allRows.error) {
        return res.status(500).json({
            errors: ["Issue talking to the database"]
        })
